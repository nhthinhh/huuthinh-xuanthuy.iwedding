(function ($) {
  ...
  const WISH_API_URL =
    'https://script.google.com/macros/s/AKfycbxJ4O24yaDHAh4w3pu33uLieBUn5PGi1IKpFI-8OoFgiFWl44QxwMIG31BBs99gvzxz/exec';
  ...
  function renderWishItem(name, content, timestamp) {
    var timeStr = formatWishTime(timestamp);
    return (
      '<div class="wish-box-item">' +
      '<strong>' +
      escapeHtml(name) +
      '</strong>' +
      (timeStr ? '<small class="wish-time">' + timeStr + '</small>' : '') +
      '<p>' +
      escapeHtml(content) +
      '</p></div>'
    );
  ...
  function loadWishes() {
    if (!$('.wish-box').length || !WISH_API_URL) return;
    $.getJSON(WISH_API_URL, function (res) {
      if (!res || res.error || !res.wishes) return;
      var html = res.wishes
        .map(function (w, idx) {
          return renderWishItem(w.name, w.content, w.timestamp);
        })
        .join('');
      $('.wish-box').append(html);
    });
  }
  ...
  if ($('#wish-form').length) {
    $('#wish-form').validate({
      ...
      submitHandler: function (form) {
        const buttonSend = $('button.theme-btn');
